...
7. How many operands does a RISC-V R-type instruction specify? What about an I-type (except for LUI and AUIPC)?
...
10. Using the following RISC-V assembly, translate the following instructions to machine code. The register numbers have been provided as a comment.
  * `auipc a0, 0 # (reg[10] = 0 + pc (4))`
  * `add t0, t0, t1 # reg[5] = reg[5] + reg[6]`
11. Using the following machine code provided in hex, translate the machine code to RISC-V assembly. You can specify just the register numbers instead of the standardized names.
  * `0x00858593`
  * `0x00e51513`
...
14. The MUX in the previous problem was controlled by a control signal from the control unit. In English, explain what the control signal that controls this MUX means, and why it would be 0 or 1 for a specific instruction. Is it possible that the value of the control signal doesn't matter at all? Why or why not?
15. In lecture, when we made the change to implement branch-type instructions in the datapath, why did we need to use an additional adder? Why couldn't we use the adder we had previously, or even the ALU?
